      ...
      body: ordersAsync.when(
        data: (orders) => diagnosisAsync.when(
            ...
            for (final report in reports) {
              timeline.add(
                _TimelineItem(
                  ...
                  type: 'diagnosis',
                  title:
                      'فحص: ${report.issueDescriptionAr.length > 30 ? '${report.issueDescriptionAr.substring(0, 30)}...' : report.issueDescriptionAr}',
                  subtitle: report.workshop?['name_ar'] ?? '',
                  amount: report.laborQuote != null
          ...
          error: (error, stackTrace) =>
              const OcErrorState(message: 'تعذر تحميل البيانات'),
        ),
        loading: () => const Center(child: CircularProgressIndicator()),
