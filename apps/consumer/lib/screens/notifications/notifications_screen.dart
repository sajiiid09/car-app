                  ...
                  return GestureDetector(
                      ...
                      switch (n.type) {
                        ...
                        case 'diagnosis':
                          if (data.containsKey('report_id')) {
                            context.push('/diagnosis/${data['report_id']}');
                          }
                          break;
                        case 'workshop_bill':
                          if (data.containsKey('bill_id')) {
                            context.push('/bill/${data['bill_id']}');
                          }
                          break;
                        case 'review':
                          if (data.containsKey('order_id')) {
                            context.push('/rate/${data['order_id']}');
                          }
                          break;
                      }
                    },
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: n.isRead
                            ? OcColors.surfaceCard
                            : OcColors.primary.withValues(alpha: 0.08),
                        borderRadius: BorderRadius.circular(OcRadius.md),
                        border: Border.all(
                          color: n.isRead
                              ? OcColors.border
                              : OcColors.primary.withValues(alpha: 0.3),
                        ),
                      ),
                      child: Row(
